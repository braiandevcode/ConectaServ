...
import ProfileTaskerInfo from './ClientRole/ProfileTaskerInfo';
...
const AllSevices = () => {
  ...
  // CUSTOM HOOKS
  const { taskerData, accessToken, selectedTaskerProfile, setSelectedTaskerProfile } = useMain();
  ...
  const handleClickGetTaskerProfile = async (idTasker: string): Promise<void> => {
    if (!accessToken) return; //NO SEGUIR

    setLoadingTaskerId(idTasker);
    
    // MANEJAR LOADER DEL BOTON ACA
    try {
      // setLoading(true)
      const taskerResponse: TDataPayloadTaskerSingle | null = await getDetailsTasker({ accessToken, idTasker });
      if (!taskerResponse) return;
      setSelectedTaskerProfile(taskerResponse);
    } catch (error) {
        const err = error as iStatusError;
        openGlobalModal(EModalGlobalType.MODAL_ERROR);
        showError('Ups', 'Ocurrio un error inesperado, intente nuevamente')
       throw err;
    } finally {
      // setLoading(false)
      ...
      {taskerData.length === 0 ? (
        ...
        <div className='main-content c-flex c-flex-column'>
          {selectedTaskerProfile ? (
            <ProfileTaskerInfo profileData={selectedTaskerProfile} isEditable={false} onBackToList={() => setSelectedTaskerProfile(undefined)} />
          ) : (
            <>
              <div>
                      ...
                      <div className='card__content'>
                        ...
                        <div className='card__button--container' id={`${service.tasker.idTasker}`}>
                          {loadingTaskerId === service.tasker.idTasker ? (
                            ...
                            <Button type='button' variant='btn btn__info' onClick={() => handleClickGetTaskerProfile(service.tasker.idTasker)}>
                              Ver perfil
                            </Button>
                          )}
                        </div>
