...
const { USER, USER_IDENTIFY, USER_CODE_REQUEST, USER_VERIFY, AUTH_LOGIN, LOGOUT} = endPointUser; //DESESTRUCTURAR ENDPOINT
...
const useUserApi = () => {
  ...
  const userVerify = async ({ email, code, token }: iDataVerifyCode): Promise<iMessageResponseStatus>  =>{
    // INTENTAR ENVIO
    try {
  ...
  const signIn = async (setter: TStateAuth & TDataLoginUser):Promise<void> => {
    const { setIsAuth, setAccessToken, setError, password, userName } = setter;
    try {
      setIsSending(true);
      setLoading(true);
  ...
  const logout = async (setter: Omit<TStateAuth, 'setError'>):Promise<void> => {
  ...
  return { getIdentifyEmail, addUser, signIn, sendCodeToUserEmail, userVerify, logout }; // ==> ACA RETORNO TODOS LOS METODOS QUE HACEN REFERNECIA A DATOS DE USUARIOS
};
