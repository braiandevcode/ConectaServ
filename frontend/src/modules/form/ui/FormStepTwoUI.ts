...
export default class FormStepTwoUI implements iBuildStepUI {
  ...
  public async buildStep({ formRegisterUI }: { formRegisterUI: FormRegisterUI; formStepUI?: FormStepUI }): Promise<HTMLDivElement> {
    ...
    inputProfileInstance.onChange(async (files) => {
      ...
      // GUARDAR EN STORAGE
      this.formRegister.saveDataStepPersistence({ step: 2, elements: [$INPUT_UPLOAD_PROFILE, textareaInstance.render() as HTMLTextAreaElement] });
    });
    ...
    inputExpInstance.onChange(async (files) => {
      ...
      this.formRegister.saveDataStepPersistence({ step: 2, elements: [$INPUT_UPLOAD_EXP, textareaInstance.render() as HTMLTextAreaElement] });
    });
    ...
    const btnInstanceNext: ButtonNext = this.formRegister.createBtnNext(); //INSTANCIAR BOTON NEXT

    //SUSCRIBIR CLICK EN BOTON
