...
export default class FormStepThreeUI implements iBuildStepUI {
  ...
  public buildStep({ formRegisterUI }: { formRegisterUI: FormRegisterUI; formStepUI?: FormStepUI }): HTMLDivElement {
    ...
    const createBtnInstanceNext: ButtonBaseUI = this.formRegister.createBtnNext(); //INSTANCIAR BOTON SIGUIENTE
    ...
    filterByNameOfReinsert.forEach((ir) => {
      // SUSCRIBIMOS A LAS INSTANCIAS SU ONCHANGE
      ir.onChange((val) => {
        this.formRegister.saveDataStepPersistence({ step: this.formRegister.getStepForm(), elements: [ir.render() as HTMLInputElement, ...this.optionGroupBuilder.getInputsChecks()] });
      });
    });
  ...
  private handlerSelectBudge(val: string, instanceAmount: InputField, btn: ButtonBaseUI): void {
    ...
    if (val === 'yes') {
      ...
      // GUARDAR NUEVAMENTE
      this.formRegister.saveDataStepPersistence({ step: this.formRegister.getStepForm(), elements: [instanceAmount.render() as HTMLInputElement, ...this.optionGroupBuilder.getInputsChecks()] });
    }
  }
  ...
  private handlerBudgeAmount(val: string, inputAmountInstance: InputField): void {
    ...
    if (result) {
      ...
      this.formRegister.saveDataStepPersistence({ step: this.formRegister.getStepForm(), elements: [inputAmountInstance.render() as HTMLInputElement, ...this.optionGroupBuilder.getInputsChecks()] });
    }
  }
}
