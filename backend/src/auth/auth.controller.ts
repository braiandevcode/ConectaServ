import { Controller, Post, Req, UseGuards, Body, Res, HttpCode, HttpStatus } from '@nestjs/common';
...
export class AuthController {
  ...
  async login(@Res({ passthrough: true }) res: Response,@Req() req: Request): Promise<iAccessToken | null> {
    ...
    const data: { accessToken: string; refreshToken: string } | null= await this.authService.signIn(
      user,
      ip,
      userAgent,
    );
  ...
  async refresh(@Req() req: Request): Promise<iAccessToken> {
    //OBTENER EK REGISTRO VALIDADO DE LA  DB QUE DEVUELVE LA ESTRATEGIA
